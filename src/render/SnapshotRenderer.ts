...
/**
 * Snapshot を盤面領域に描画する（モード非依存・盤面サイズ非依存）。
...
export class SnapshotRenderer {
  ...
  drawBoard(
    ...
    const { cellSize, boardX, boardY, cols, rows } = layout;
    ...
    // 盤面背景（縦グラデーション）。
    this.ctx.globalAlpha = opts.boardOpacity;
    ...
    roundRect(this.ctx, boardX, boardY, w, h, 8);
    this.ctx.clip();
    if (opts.grid) this.drawGridLines(layout);

    const style = getSkin().style ?? 'gradient';
  ...
  private drawSingleCell(
    ...
    if (cell.type === 'ghost') {
      if (showGhost) drawGhost(this.ctx, px, py, size, colorOf(cell.id));
      return;
    }

    drawCell(this.ctx, px, py, size, colorOf(cell.id), 1, style);
  ...
  private drawGridLines(layout: BoardLayout): void {
    ...
    for (let x = 1; x < cols; x++) {
      ...
      this.ctx.lineTo(boardX + x * cellSize, boardY + rows * cellSize);
      this.ctx.stroke();
    }
    for (let y = 1; y < rows; y++) {
      this.ctx.beginPath();
      this.ctx.moveTo(boardX, boardY + y * cellSize);
      this.ctx.lineTo(boardX + cols * cellSize, boardY + y * cellSize);
      this.ctx.stroke();
    }
    this.ctx.globalAlpha = 1;
  }
