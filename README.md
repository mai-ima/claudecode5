...
- **AIと対戦（テトリス）** … 内蔵CPUとオフライン対戦
- **オンライン対戦** … Vercel サーバーレス（/api + KV）経由（疎結合・HTTP ポーリング）
...
- レスポンシブ描画（盤面サイズから動的算出。テトリス10×20／ぷよ6×12を正しく描画、2 盤面も自動レイアウト）。
- 画面下に操作説明を常時表示。役名バナー・カウントダウン演出。
